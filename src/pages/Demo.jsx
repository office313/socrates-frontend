  ...
  return (
    <div style={{ background: 'var(--blue-light)', borderRadius: 10, padding: '14px 16px', margin: '0 0 22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--blue-dark)', fontSize: 14, fontWeight: 600 }}>
        <Calendar size={18} /> Miércoles · 10:00 a.m. <span style={{ fontWeight: 400, color: '#6b7280' }}>(hora de Panamá)</span>
      </div>
...
export default function Demo() {
  ...
  const [estado, setEstado] = useState('cargando')   // cargando | error | form | enviando | ok
  ...
  useEffect(() => {
    ...
    axios.get('/api/demo/info', { params: { token } })
      .then(r => { setEmpresa(r.data.empresa); setEstado('form') })
        ...
        <div style={body}>
          ...
          {estado === 'ok' && (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <p style={{ fontSize: 15, color: '#374151', lineHeight: 1.6, margin: '0 0 22px' }}>{resultado?.mensaje}</p>
              <InfoDemo />
